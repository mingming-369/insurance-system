...
def edit_warranty(request, pk):
    ...
    # 3. Logic to calculate the current duration (Years)
    current_duration = 5 # Default
    if warranty.start_date and warranty.end_date:
        # We add 1 day because the logic is (Start + Duration - 1 Day)
        # Example: Jan 1 to Dec 31 is 365 days. 365 + 1 = 366. 366 / 365.25 ≈ 1 Year.
        days_diff = (warranty.end_date - warranty.start_date).days + 1
        current_duration = int(round(days_diff / 365.25))
    
    # 4. Logic to detect if the duration is "Other"
    ...
    if request.method == 'POST':
        ...
        if product_select == 'other':
            # If user selected "Other...", take the value from the text input
            final_product_name = request.POST.get('product_other')
        else:
            # If user selected "Inverter", use that value directly
...
def delete_defect_record(request, pk):
    ...
    return render(request, 'insurance_app/delete_defect_record_confirm.html', {'defect': defect})
